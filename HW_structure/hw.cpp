...
struct book_shop {
	int year, c_str, art;
	std::string author, title, publish;
};
struct BGYK {
	int fiber, fats, carb;
	float cal;
...
struct fio {
	std::string name, surname, patr;
};
struct copybook {
	std::string manufac, line_type;
	int price;
};
struct time {
...
float calculateCost(struct Dimension sizes, float cupboardCost, float consumption, float paintCost,
	...
	float cupboard_cost = calculateCupboardCost(sizes, cupboardCost);
	float painting_cost = calculatePaintingCost(sizes, consumption, workCost, paintCost);
	float delivery_cost = calculateLiftingCost(sizes, weight, floorsCount, riseCost) + deliveryCost;
	float full_cost = painting_cost + cupboard_cost + delivery_cost + deliveryCost;
	printf("Cost by position:\n  - cupboard cost:%.2f\n  - painting cost:%.2f\n  - delivery cost(with rise)%.2f:\nTotal cost: %.2f",
		cupboard_cost, painting_cost, delivery_cost, full_cost);
